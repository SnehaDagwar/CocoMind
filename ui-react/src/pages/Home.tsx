    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div className="glass-card p-8 lg:col-span-3">
          <h2 className="text-xl font-display font-bold text-slate-100 mb-4 flex items-center gap-2">
            <span>🎯</span> What CocoMind Does
          </h2>
          <p className="text-slate-400 leading-relaxed mb-6">
            CocoMind reads a CRPF tender (NIT) + bidder submissions and produces 
            <strong className="text-primary-400 font-semibold mx-1">criterion-by-criterion, source-pinned, RTI-auditable eligibility verdicts</strong> 
          ...
          <div className="bg-surface/50 rounded-xl p-5 border border-white/5">
            <h3 className="text-sm font-bold text-slate-300 mb-3 uppercase tracking-wider">Hard non-negotiables:</h3>
        ...
        <div className="glass-card p-8 lg:col-span-2">
          ...
          <div className="space-y-4">
            {stages.map((stage) => (
              <div key={stage.name} className="flex items-center gap-4 p-3 rounded-lg hover:bg-white/5 transition-colors group">
                <div className="w-10 h-10 rounded-full bg-primary-500/10 text-primary-400 flex items-center justify-center group-hover:bg-primary-500/20 group-hover:scale-110 transition-all">
      ...
      <div>
        <h2 className="text-2xl font-display font-bold text-slate-100 mb-6 flex items-center gap-2">
          <span>🗂️</span> 3 Demo Bid Packages
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {bids.map((bid) => (
            <div key={bid.title} className={`glass-card p-6 border-t-4 ${
              bid.color === 'emerald' ? 'border-t-emerald-500' :
              bid.color === 'red' ? 'border-t-red-500' : 'border-t-amber-500'
            }`}>
              <div className="text-3xl mb-3">{bid.icon}</div>
              <h3 className={`text-lg font-bold mb-1 ${
                bid.color === 'emerald' ? 'text-emerald-400' :
                bid.color === 'red' ? 'text-red-400' : 'text-amber-400'
              }`}>{bid.title}</h3>
              <p className="text-xs font-semibold text-slate-400 mb-4">{bid.company}</p>
              <p className="text-sm text-slate-300 leading-relaxed mb-6">{bid.desc}</p>
